...
//        }
    ...
    ///Round corners with a much lighter function. Works on layers, should be preffered over roundCorner.
    ...
    func drawShadow(shadowColor:UIColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1), shadowOpacity:Float = 0.07, shadowPath:UIBezierPath? = nil, shadowRadius:Float = 5, cornerRadius:Float = 5.0, offset: CGSize = CGSize(width: 1, height: 1)) {
    ...
    func createShadow(shadowColor:UIColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1), shadowOpacity:Float = 0.07, offset: CGSize = CGSize(width: 1, height: 1), shadowRadius:Float = 5, cornerRadius:Float = 5.0) {
        self.layer.masksToBounds = false
        self.layer.shadowColor  = shadowColor.cgColor
        self.layer.shadowOpacity = shadowOpacity
        self.layer.shadowOffset = offset
    ...
    func drawShadowDifferently(shadowColor:UIColor = #colorLiteral(red: 0, green: 0, blue: 0, alpha: 1), shadowOpacity:Float = 0.07, shadowPath:UIBezierPath? = nil, shadowRadius:Float = 5, cornerRadius:Float = 5.0, offset: CGSize = CGSize(width: 1, height: 1)) {
        self.layer.masksToBounds = false
        self.layer.shadowColor  = shadowColor.cgColor
        self.layer.shadowOpacity = shadowOpacity
        self.layer.shadowOffset = offset
        ...
        self.layer.shadowPath   = shdwpath!.cgPath
        self.layer.edgeAntialiasingMask = CAEdgeAntialiasingMask.layerBottomEdge
    }
...
extension UIView {
    func applyGradient(colours: [UIColor]) -> Void {
        self.applyGradient(colours: colours, startPoint: nil, endPoint: nil)
    }
    
    func applyGradient(colours: [UIColor] = [UIColor.black], maskedCorners: CACornerMask = [.layerMinXMinYCorner, .layerMaxXMaxYCorner, .layerMaxXMinYCorner, .layerMinXMaxYCorner], cornerRadius: CGFloat = 0.0, startPoint: CGPoint?, endPoint: CGPoint?) -> Void {
        let gradient: CAGradientLayer = CAGradientLayer()
        gradient.frame = self.bounds
        gradient.colors = colours.map { $0.cgColor }
        guard let startPointer = startPoint, let endPointer = endPoint else { return }
        gradient.startPoint = startPointer
        gradient.endPoint = endPointer
        gradient.maskedCorners = maskedCorners
        gradient.cornerRadius = cornerRadius
        if let sublayers = layer.sublayers {
            for item in sublayers where item is CAGradientLayer {
    ...
    func getUnderline(_ color: UIColor = #colorLiteral(red: 0.8784313725, green: 0.137254902, blue: 0.2588235294, alpha: 1)) {
        let border = CALayer()
        let width = CGFloat(1.0)
        border.borderColor = color.cgColor
        border.frame = CGRect(x: 0, y: self.frame.size.height - width, width:  self.bounds.size.width, height: 1)
        border.borderWidth = width
        if let sublayers = layer.sublayers {
            for item in sublayers where item is CATransformLayer {
                item.removeFromSuperlayer()
            }
        }
...
extension UIView {
    func addBottomBorderWithColor(color: UIColor, height: CGFloat) {
        ...
        border.frame = CGRect(x:0, y:self.frame.size.height - height - 7.5, width:self.frame.size.width, height:height)
        self.layer.addSublayer(border)
    }
