...
extension UIImageView {
    ...
    func isPlaceholderImage(_ placeHolderImage: UIImage = UIImage(imageLiteralResourceName: "icAddNewimage")) -> Bool {
...
extension UIImage{
    ...
    convenience init?(gradientColorsHorizontal:[UIColor], size:CGSize = CGSize(width: 10, height: 10), locations: [Float] = [])
    {
        UIGraphicsBeginImageContextWithOptions(size, false, 0)
        let context = UIGraphicsGetCurrentContext()
        
        let colorSpace = CGColorSpaceCreateDeviceRGB()
        let colors = gradientColorsHorizontal.map {(color: UIColor) -> AnyObject? in return color.cgColor as AnyObject? } as NSArray
        let gradient: CGGradient
        if locations.count > 0 {
            let cgLocations = locations.map { CGFloat($0) }
            gradient = CGGradient(colorsSpace: colorSpace, colors: colors, locations: cgLocations)!
