...
//        self.insulinCountLbl.text =  "\(BleManager.sharedInstance.reservoirLevelData)".isEmpty ? "-- Units" :  "\(BleManager.sharedInstance.reservoirLevelData) Units"
    ...
    public func setUpAttributedString(){
        let attributedString = NSMutableAttributedString(string: "\(BleManager.sharedInstance.reservoirLevelData)".isEmpty ? "--" : (BleManager.sharedInstance.reservoirLevelData == "-1" ? "--" : "\(BleManager.sharedInstance.reservoirLevelData)") , attributes: [
